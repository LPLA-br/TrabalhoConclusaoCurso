...
\begin{abstract}
  ...
  Computers emerged in the 20th century as machines that enabled the evolution
  of how information generated in modern human societies is handled. The
  vertiginous increase in computing power allowed the consolidation of
  increasingly complex systems, which implied a change in how software is
  written. From programs entirely conceived by a programmer to large systems
  maintained by several technicians whose foundations are born externally in
  the form of modules and packages. However, the agility provided by modularity
  brought with it the possible careless use of dependencies that make it
  functional, implying problems that accumulate over time. Based on this, an
  experimental object-oriented application was created for sewing workshops
  using the hexagonal architecture with adapter classes as a way to invert
  dependencies in the communication and data parts. Finally, it was concluded
  that efforts towards independence between the system and dependencies are
  possible, but also costly in terms of time and intellectual resources, and
  can cause overengineering.

  \textbf{Keywords: Object-Oriented Programming, Dependency Management, Patterns, Architecture}
\end{abstract}
}
\newpage
